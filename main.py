...
def run_simulation_logic(simulation: Simulation, event_bus: EventBus, demand_engine: DemandEngine, cities):
    ...
    print(f"{Color.BOLD}=== Starting simulation with {len(simulation.factories)} factories, {len(cities)} cities ==={Color.RESET}")
...
#     global_state.cities = cities
    ...
    # Phase 11: 5 Factories strategically located
                ...
                for factory in factories:
                    ...
                    try:
                        ...
                        for queue_name in ['Crushed', 'Molded', 'Filled', 'QC']:
                            ...
                            if queue_id:
                                # Get queue object
                                if queue_name == 'Crushed' and hasattr(factory, 'q_crushed'):
                                    insert_queue_snapshot(queue_id, factory.q_crushed.size())
                                elif queue_name == 'Molded' and hasattr(factory, 'q_molded'):
                                    insert_queue_snapshot(queue_id, factory.q_molded.size())
                                elif queue_name == 'Filled' and hasattr(factory, 'q_filled'):
                                    insert_queue_snapshot(queue_id, factory.q_filled.size())
                                elif queue_name == 'QC' and hasattr(factory, 'q_qc'):
                                    insert_queue_snapshot(queue_id, factory.q_qc.size())
                    except Exception as e:
                        print(f"Error logging snapshots: {e}")
        
        snapshot_thread = threading.Thread(target=log_snapshots, daemon=True, name="DB-Snapshots")
    ...
    demand_engine = DemandEngine(
        ...
        routing_strategy=BalancedRoutingStrategy(proximity_weight=0.6, workload_weight=0.4),
        db_manager=db_manager,  # Phase 13: Pass for shipment logging
    )
    print(f"{Color.CYAN}DEBUG: DemandEngine initialized with {len(factories)} factories, {len(cities)} cities{Color.RESET}")
    ...
    run_textual_dashboard(simulation, factories, event_bus, cities=cities, demand_engine=demand_engine)
